// Migración de credenciales — ciudad_del_campo_ii → Better Auth (Convex)
// Uso:  SRC_DB='postgres://...ciudad_del_campo_ii?sslmode=verify-full' node migrate/ciudad-del-campo-auth.mjs
// Importa los hashes de contraseña existentes (scrypt de Better Auth) sin reset.
// Los usuarios/unidades de Ciudad del Campo II ya están migrados: al primer
// login, ensureProfile enlaza el perfil existente (por email) con esta credencial.
...
async function main() {
  ...
  if (!url) throw new Error("Falta SRC_DB (cadena a la base ciudad_del_campo_ii)");
  ...
  console.log(
    `✅ Credenciales Ciudad del Campo II: creadas=${created} omitidas=${skipped} errores=${errors}`,
  );
}
